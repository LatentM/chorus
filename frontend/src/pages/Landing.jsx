...
/* the three edges on the authentication path, leaf -> root */
const PATH = [
...
function MerkleFigure() {
  return (
    ...
    >
      {DARK.map((e, i) => (
        <path key={`d${i}`} d={e.d} className="tree-edge" />
      ))}
      ...
      {PATH.map((e, i) => (
        <path
          key={`l${i}`}
          d={e.d}
          className="tree-edge-lit"
          style={{ "--len": e.len, animationDelay: `${0.35 + i * 0.55}s` }}
        />
      ))}

      {/* leaves: registered voters. Everyone is a solid block except you. */}
      {LEAF_X.map((x, i) =>
        i === ME ? (
      ...
      <text x={ROOT_X} y={Y.root - 18} textAnchor="middle" className="tree-caption tree-caption-lit">
        root · stored on-chain
      </text>
      <text x={LEAF_X[ME]} y={Y.leaf + 24} textAnchor="middle" className="tree-caption">
        your leaf
      </text>
      <text x={LEAF_X[ME]} y={Y.leaf + 36} textAnchor="middle" className="tree-caption">
        ███████
      </text>
    </svg>
  );
  ...
  return (
    <div className="space-y-20 pb-6">
      ...
      <section className="grid lg:grid-cols-[1.05fr_1fr] gap-10 lg:gap-14 items-center">
        <div>
          <p className="font-mono text-[11px] tracking-[0.22em] text-muted uppercase mb-5">
          ...
          <p className="text-muted leading-relaxed max-w-xl">
            TrustVote runs elections where no one — not even the operator — can see a
          ...
          <div className="flex flex-wrap gap-3 mt-8 mb-10">
            ...
            <button className="btn-ghost" onClick={() => onNavigate("verifier")}>
              Audit an election
            </button>
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ["~2s", "proof built in-browser"],
              ["1", "ballot per voter, on-chain"],
              ["0", "parties you must trust"],
            ].map(([n, l]) => (
              <div key={l} className="border-l-2 border-edge pl-3.5">
                <div className="stat-v">{n}</div>
                <div className="font-mono text-[10px] uppercase tracking-[0.14em] text-muted mt-1.5 leading-snug">
                  {l}
                </div>
              </div>
            ))}
        ...
        <div className="card">
          ...
          <p className="text-sm text-muted mt-4 border-t border-edge pt-4 leading-relaxed">
            The circuit proves your leaf sits somewhere under this root. It never
            outputs which leaf.
          </p>
        </div>
      </section>
      ...
      <section>
        <div className="flex items-baseline justify-between border-b border-edge pb-4 mb-8 gap-4">
          ...
          <span className="font-mono text-[11px] uppercase tracking-[0.16em] text-muted hidden sm:inline">
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {GUARANTEES.map((g) => (
            <div key={g.p} className="card flex flex-col">
              ...
              <p className="font-mono text-[11px] text-foil mt-5 pt-4 border-t border-edge tracking-wide">
                {g.p}
              </p>
            </div>
          ))}
      ...
      {/* ——— How an election runs. Numbered because the order is load-bearing:
             a phase cannot be re-entered once passed. ——— */}
      <section>
        <div className="border-b border-edge pb-4 mb-8">
          <h3 className="font-display font-black text-2xl">How an election runs</h3>
          <p className="text-sm text-muted mt-2">
            Four phases, in order. A phase cannot be re-entered once passed.
          </p>
        </div>
          ...
          {STEPS.map(([t, d], i) => (
            <li key={t} className="bg-panel p-5">
              <div className="flex items-center gap-2.5 mb-3">
                <span className="font-mono text-[11px] text-foil">
      ...
      {/* ——— Honest limits. A reviewer will ask; answering first reads as
             command of the material rather than as a gap. ——— */}
        ...
        <div className="grid md:grid-cols-2 gap-x-10">
          {LIMITS.map(([k, v]) => (
            <div key={k} className="kv py-2 border-b border-edge/60">
              ...
              <span className="kv-v !font-normal">{v}</span>
            </div>
          ))}
        </div>
        ...
        <div className="md:flex items-start justify-between gap-8">
          <div className="max-w-xl">
            ...
            <p className="text-sm text-muted leading-relaxed">
              The platform operator can delegate a single election to your agency's
              wallet — you monitor it and publish its results; authority never extends
              further. Results are refused before the polls close and timestamped
              on-chain the moment they appear, so nobody can claim an early or altered
              count.
            </p>
          </div>
          <button className="btn-ghost mt-5 md:mt-1 shrink-0" onClick={() => onNavigate("admin")}>
            Open the admin desk
          </button>
        </div>
      </section>
